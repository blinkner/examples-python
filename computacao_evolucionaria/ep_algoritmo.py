...
n_filhos = 30 #número de filhos
sigma = 0.1 # desvio padrão
lim_sup = 2 # limite superior
lim_inf = -1 # limite inferior
...
for i in range(popsize):
    pop[i] = np.random.uniform(lim_inf, lim_sup) # inicialização da população
...
while (gen<genmax):
    ...
    for i in range(n_filhos):
        ...
        pop_f[i] = pop[p1] + sigma * np.random.normal(0, 1) # faz a mutação normal
