...
nbits = 15 # número de bits que define um indivíduo.
...
pc = 0.725 # limiar taxa de cruzamento. Sugestão 0.65 <= pc <=0.8
pm = 0.031 # limiar taxa de mutação. Sugestão: 0.01 <= pm <= 0.05
n = 3 # número de cromossomos
lim_sup = np.array([15, 15, 15]) # limites superiores
...
while execucao <= max_execucao:
    ...
    for i in range(popsize):
        ...
        f[i] = cf.simulacao_malha(pop_10[i][0], pop_10[i][1], pop_10[i][2]) #Calculando a função custo
    ...
    while (gen<genmax and cont_limite<nlimite):
        ...
        for i in range(popsize):
            ...
            ff[i] = cf.simulacao_malha(pop_10[i][0], pop_10[i][1], pop_10[i][2]) # calculando a função custo
    ...
    dados.append(str(pop_10[pos][1])) # Ti
    dados.append(str(pop_10[pos][2])) # Td
